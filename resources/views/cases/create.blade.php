...
{{ Form::open(['route' => 'cases.store', 'method' => 'post', 'class' => 'frm-submit-data', 'enctype' => 'multipart/form-data']) }}
<div class="row">
        ...
        <div class="card shadow-none rounded-0 border">
            <div class="card-body">
                <div class="row">
                   ...
                   {!! Form::label('contract_id', __('Contract'), ['class' => 'form-label required-sym']) !!}
                                     <select class="form-control choices-js" id="choices-js" name="contract_id">
                                <option value="">{{ __('Select Contract') }}</option>
                                @foreach($contracts as $contract)
                                    <option value="{{ $contract->id }}" {{ ($contract->id==$contract_id) ? 'selected': '' }}>{{ $contract->contract_no }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            {!! Form::label('branch', __('Branch'), ['class' => 'form-label required-sym']) !!}
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.status == 1) {
                    ...
                    $('#highcourt').append('<option value="">{{ __('
                    ...
                    $('#casetype_div').removeClass('d-none').append(
                        ...
                        '</label><select class="form-control" name="casetype" id="casetype"> <option value="">{{ __('
                        Please Select ') }}</option><option value="Case Number">{{ __('
                        Case Number ') }}</option><option value="Diary Number">{{ __('
                        Diary Number ') }}</option></select>'
                    );
